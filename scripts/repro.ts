...
async function main() {
  ...
  await runScenario("3. Змішування валют", async () => {
    // Після фіксу: помилка через різні валюти.
    // До фіксу: 100 USD просто додавалися як 100 EUR до рахунку Bob.
    const res = await transferMoney({
  ...
  await runScenario("4. Крадіжка з чужого рахунку", async () => {
    ...
    // Після фіксу: помилка доступу.
    // До фіксу: гроші списувались у Bob і йшли Alice.
    const res = await transferMoney({
      fromAccountId: "acc-bob",
      toAccountId: "acc-alice",
      amount: 100,
  ...
  await runScenario("5. Стан гонитви", async () => {
    ...
    // Після фіксу: 5 пройдуть (баланс стане 0), 5 відхиляться.
    // До фіксу: всі 10 можуть пройти, баланс пробиває нуль і йде в -1000.
